...
(function(){
  ...
  theme.buildProductImage = function(product, imageSizes) {
    ...
    function addImageProperties(imageEl, imageSizes) {
      ...
      // imageEl.src = product.images[2];
      imageEl.srcset = product.images[2];
...
(function() {
  ...
  function openModal(id, opener) {
    ...
    if (!modal) return console.warn('[wmodal] Missing modal with id:', id);

    modal.setAttribute('aria-hidden', 'false');
    body.style.overflow = 'hidden';

    const dialog = modal.querySelector('.wmodal__dialog');
    const backdrop = modal.querySelector('.wmodal__backdrop');
    const closeBtns = modal.querySelectorAll('[data-wmodal-close]');
    ...
    closeBtns.forEach(btn => btn.addEventListener('click', closeModal, { once: true }));
  ...
  // Delegated click listener for all openers
  document.addEventListener('click', (e) => {
    const opener = e.target.closest('[data-wmodal-open]');
    if (!opener) return;
    e.preventDefault();
    const id = opener.getAttribute('data-wmodal-open');
    openModal(id, opener);
  });

  // Ensure modal is hidden on load and section reloads
  function ensureHidden(root) {
    root.querySelectorAll('.wmodal').forEach(m => 
      m.setAttribute('aria-hidden', m.getAttribute('aria-hidden') || 'true')
    );
  }
  document.addEventListener('DOMContentLoaded', () => ensureHidden(document));
  document.addEventListener('shopify:section:load', (e) => ensureHidden(e.target));
  document.addEventListener('shopify:section:select', (e) => ensureHidden(e.target));
})();
